...
- (void)warnInUse {
    ...
    if (_crashOnErrors) {
        NSString *tipStr = [NSString stringWithFormat:@"The FMDatabase %@ is currently in use.", self];
        NSAssert(false,tipStr );
        abort();
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    #ifndef NS_BLOCK_ASSERTIONS
        if (_crashOnErrors) {
            NSString *tipStr = [NSString stringWithFormat:@"The FMDatabase %@ is not open.", self];
            NSAssert(false, tipStr);
            abort();
        }
    #endif
...
- (FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (!pStmt) {
        ...
        if (SQLITE_OK != rc) {
            ...
            if (_crashOnErrors) {
                NSString *tipStr = [NSString stringWithFormat:@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]];
                NSAssert(false, tipStr);
                abort();
            }
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (rc == SQLITE_ROW) {
        NSString *tipStr = [NSString stringWithFormat:@"A executeUpdate is being called with a query string '%@'", sql];
        NSAssert(NO, tipStr);
    }
